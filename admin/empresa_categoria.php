...
<article>
	...
	$existe = mysqli_num_rows(mysqli_query($lnk,"SELECT * FROM category WHERE id='$id'"));
	if($existe){extract(mysqli_fetch_array(mysqli_query($lnk,"SELECT * FROM category WHERE id='$id'")));}
	...
	<div class="conteudo">
		<div class="linha">
			<div class="titulo">
				...
				<div class="ponto"></div>
				<a href="/admin/empresa_categorias">Categorias</a>
				<div class="ponto"></div>
				<a href="">Categoria</a>
			</div>
		</div>
				...
				<div class="corpo">
					...
					<div id="INF2" class="corpoCima none">
						<div class="grupo">
							<div class="grupoEsq">Nome:</div>
							<div class="grupoDir">
								<input type="text" class="inP" name="nome_en" value="<? echo $nome_en;?>">
							</div>
						</div>
						<div class="clear"></div>			
					</div>
					<div id="INF3" class="corpoCima none">
						<div class="grupo">
							<div class="grupoEsq">Nome:</div>
							<div class="grupoDir">
								<input type="text" class="inP" name="nome_fr" value="<? echo $nome_fr;?>">
							</div>
						</div>
						<div class="clear"></div>			
					</div>
					<div id="INF4" class="corpoCima none">
						<div class="grupo">
							<div class="grupoEsq">Nome:</div>
							<div class="grupoDir">
								<input type="text" class="inP" name="nome_es" value="<? echo $nome_es;?>">
							</div>
						</div>
						<div class="clear"></div>			
					</div>
					<div class="corpoBaixo">
						...
						<button type="button" class="btA" name="cancelar" onClick="window.location.href='/admin/empresa_categorias';">CANCELAR</button>					
					</div>
					</form>
				</div>
			</div>
...
<div id="GUARDAR" class="modal">
	...
	<div class="modalSize">
		...
		<div class="modalFoot">
			...
			<button class="btC modalBt" name="cancelar" onClick="window.location.href='/admin/empresa_categorias';">VOLTAR</button>
		</div>
	</div>
</div>
<!-- -->
...
$(document).ready(function (e) {
	$("#FORMULARIO").on('submit',(function(e) {
		e.preventDefault();
		$.ajax({
			url: "/admin/_empresa/categoria.php",
			type: "POST",
			...
			success: function(data){
				...
				if(data){
					mostrar('GUARDAR');
					window.history.pushState("object or string", "Title", "/admin/empresa_categoria/"+data);
