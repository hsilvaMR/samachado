...
<article>
	...
	$existe = mysqli_num_rows(mysqli_query($lnk,"SELECT * FROM world WHERE id='$id'"));
	if($existe){extract(mysqli_fetch_array(mysqli_query($lnk,"SELECT * FROM world WHERE id='$id'")));}
	...
	<div class="conteudo">
		<div class="linha">
			<div class="titulo">
				<h1><? if($existe){echo "Editar";}else{echo "Novo";}?> País<h1>
			...
			<div class="coluna1">
				<div class="corpo">
					...
					<div class="corpoBaixo">
						...
						<button type="button" class="btA" name="cancelar" onClick="window.location.href='/admin/empresa_paises';">CANCELAR</button>					
					</div>
					</form>
				</div>
			</div>
...
<div id="GUARDAR" class="modal">
	...
	<div class="modalSize">
		...
		<div class="modalFoot">
			...
			<button class="btC modalBt" name="cancelar" onClick="window.location.href='/admin/empresa_paises';">VOLTAR</button>
		</div>
	</div>
</div>
<!-- -->
	...
	$("#FORMULARIO").on('submit',(function(e) {
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data){
					mostrar('GUARDAR');
					window.history.pushState("object or string", "Title", "/admin/empresa_pais/"+data);
