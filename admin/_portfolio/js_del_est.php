...
$numero = mysqli_num_rows(mysqli_query($lnk,"SELECT * FROM ficha WHERE id_estado='$id'"));
if($numero){ $aux='TF'; }
else{
	$linha = mysqli_fetch_array(mysqli_query($lnk,"SELECT * FROM estado WHERE id = '$id'"));
	$nome = $linha['nome'];
	$registo="Apagou estado ".$nome." ( #".$id." )";
	mysqli_query($lnk, "INSERT INTO registo(id_admin,id_user,registo,data,hora) VALUES ('$id_user','$id_user','$registo','$data','$hora')");

	mysqli_query($lnk, "DELETE FROM estado WHERE id='$id'");
	$aux=$id;
}
