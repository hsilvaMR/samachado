...
if($id || $nome || $nome_en || $nome_fr || $nome_es){
	if($id){
		$registo="Atualização da categoria ".$nome." ( #".$id." )";
		mysqli_query($lnk, "INSERT INTO registo(id_admin,id_user,registo,data,hora) VALUES ('$id_user','$id_user','$registo','$hoje','$hora')");
		mysqli_query($lnk,"UPDATE categoria SET nome='$nome',nome_en='$nome_en',nome_fr='$nome_fr',nome_es='$nome_es' WHERE id='$id'");
	}
	else{
		mysqli_query($lnk, "INSERT INTO categoria(nome,nome_en,nome_fr,nome_es) VALUES ('$nome','$nome_en','$nome_fr','$nome_es')");
		$id = mysqli_insert_id($lnk);
		$registo="Nova categoria ".$nome." ( #".$id." )";
		mysqli_query($lnk, "INSERT INTO registo(id_admin,id_user,registo,data,hora) VALUES ('$id_user','$id_user','$registo','$hoje','$hora')");
	}
}
echo $id;
