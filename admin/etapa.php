...
<article>
	...
	$existe = mysqli_num_rows(mysqli_query($lnk,"SELECT * FROM etapa WHERE id='$id'"));
	if($existe){extract(mysqli_fetch_array(mysqli_query($lnk,"SELECT * FROM etapa WHERE id='$id'")));}
	...
	<div class="conteudo">
		<div class="linha">
			<div class="titulo">
				<h1><? if($existe){echo "Editar";}else{echo "Nova";}?> Etapa<h1>
				<a href="/admin/painel">Painel</a>
				<div class="ponto"></div>
				<a href="/admin/etapas">Etapas</a>
				<div class="ponto"></div>
				<a href="">Etapa</a>
			</div>
		</div>
				...
				<div class="corpo">
					...
					<div class="corpoCima">
						<div class="grupo">
							...
							<div class="grupoDir">
								<input type="text" class="inP" name="descricao" value="<? echo $descricao?>" required>
							</div>
						</div>
						<div class="clear"></div>			
					</div>
	...
	$("#FORMULARIO").on('submit',(function(e) {
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data){
					mostrar('GUARDAR');
					window.history.pushState("object or string", "Title", "/admin/etapa/"+data);
				}
			}         
		});
	}));
