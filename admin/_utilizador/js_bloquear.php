...
$linha = mysqli_fetch_array(mysqli_query($lnk, "SELECT * FROM user WHERE id='$id'"));
$valor = $linha["bloqueado"];

if($valor=='1'){
	mysqli_query($lnk, "UPDATE user SET bloqueado='0' WHERE id='$id'");
	$registo="Utilizador Desbloqueado";
	mysqli_query($lnk, "INSERT INTO registo(id_admin,id_user,registo,data,hora) VALUES ('$id_user','$id','$registo','$data','$hora')");
	$aux = 0;
}else{
	mysqli_query($lnk, "UPDATE user SET bloqueado='1' WHERE id='$id'");
	$registo="Utilizador Bloqueado";
	mysqli_query($lnk, "INSERT INTO registo(id_admin,id_user,registo,data,hora) VALUES ('$id_user','$id','$registo','$data','$hora')");
	$aux = 1;
}
