...
$numero = mysqli_num_rows(mysqli_query($lnk,"SELECT * FROM tarefa WHERE id_manda='$id' OR id_faz='$id'"));
if($numero){ $aux='TF'; }
else{
	$linha = mysqli_fetch_array(mysqli_query($lnk,"SELECT * FROM user WHERE id = '$id'"));
	$nome = $linha['nome'];
	$email = $linha['email'];
	$registo="Apagou utilizador ".$nome."( #".$id." ) com o email ".$email;
	mysqli_query($lnk, "INSERT INTO registo(id_admin,id_user,registo,data,hora) VALUES ('$id_user','$id_user','$registo','$data','$hora')");
	mysqli_query($lnk, "DELETE FROM user WHERE id='$id'");
	$aux=$id;
}
