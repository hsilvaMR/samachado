...
$query = mysqli_query($lnk,"SELECT * FROM galeria WHERE id_processo='$id'");
...
$nome = $linha['nome'];
$registo="Apagou ficha de obra ".$nome."( #".$id." )";
mysqli_query($lnk, "INSERT INTO registo(id_admin,id_user,registo,data,hora) VALUES ('$id_user','$id_user','$registo','$data','$hora')");
mysqli_query($lnk, "DELETE FROM registo WHERE id_processo='$id'");

mysqli_query($lnk, "DELETE FROM galeria WHERE id_processo='$id'");
mysqli_query($lnk, "DELETE FROM processo WHERE id='$id'");
