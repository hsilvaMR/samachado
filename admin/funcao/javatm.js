...
//==================================
/*<input type="text" value="" id="text1" onkeyup="contar(1);">
<span id="cont1">0 palavra, 0 caracter</span>
$(document).ready(function() { contar(1); }); || $( window ).load(function() { contar(1); }); */
function contar(id){
	var texto = $('#text'+id).val();
	texto = texto.replace(/(\r\n|\n|\r)/g," ").trim();
	var palavras = texto.split(/\s+/g);
	palavras = palavras.length;
	texto = texto.replace(/\s*/g, "");
	var caracteres = texto.length;
	if(!texto){$('#cont'+id).html('0 palavra, 0 caracter');}
	else{
		if(palavras<2 && caracteres<2){$('#cont'+id).html(palavras+' palavra, '+caracteres+' caracter');}
		if(palavras<2 && caracteres>1){$('#cont'+id).html(palavras+' palavra, '+caracteres+' caracteres');}
		if(palavras>1 && caracteres<2){$('#cont'+id).html(palavras+' palavras, '+caracteres+' caracter');}
		if(palavras>1 && caracteres>1){$('#cont'+id).html(palavras+' palavras, '+caracteres+' caracteres');}
	}
}
function contar_tudo(id){
	var texto = $('#text'+id).val();
	var caracteres = texto.length;
	texto = texto.replace(/(\r\n|\n|\r)/g," ").trim();
	var palavras = texto.split(/\s+/g);
