...
<article>
	...
	$existe = mysqli_num_rows(mysqli_query($lnk,"SELECT * FROM estado WHERE id='$id'"));
	if($existe){extract(mysqli_fetch_array(mysqli_query($lnk,"SELECT * FROM estado WHERE id='$id'")));}
	...
	<div class="conteudo">
		<div class="linha">
			<div class="titulo">
				<h1><? if($existe){echo "Editar";}else{echo "Novo";}?> Estado<h1>
				<a href="/admin/painel">Painel</a>
				<div class="ponto"></div>
				<a href="/admin/estados">Estados</a>
				<div class="ponto"></div>
				<a href="">Estado</a>
			</div>
		</div>
...
<div id="GUARDAR" class="modal">
	...
	<div class="modalSize">
		...
		<div class="modalFoot">
			...
			<button class="btC modalBt" name="cancelar" onClick="window.location.href='/admin/estados';">VOLTAR</button>
		</div>
	</div>
</div>
<!-- -->
...
$(document).ready(function (e) {
	$("#FORMULARIO").on('submit',(function(e) {
		e.preventDefault();
		$.ajax({
			url: "/admin/_portfolio/estado.php",
			type: "POST",
			...
			success: function(data){
				...
				if(data){
					mostrar('GUARDAR');
					window.history.pushState("object or string", "Title", "/admin/estado/"+data);
